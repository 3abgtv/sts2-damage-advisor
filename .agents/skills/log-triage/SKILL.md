...
- `剩余 = 初始能量 + 回能 − 花费`；**只看 `花费` 会算出负数**，因为奇巧牌被弃会自动打出、`战术大师` 会回能。
- 搜索侧的正确性看两点：`if (effectiveCost > state.Energy) continue;` 是否存在、`Energy = Energy - cost + gain` 是否成立。
- **弃牌触发的回能也要计入 `EnergyGained`**（曾经只加了实际能量、漏了统计，导致显示为负）。
- 计划日志现在带 `耗能=/回能=`，可直接复核。
